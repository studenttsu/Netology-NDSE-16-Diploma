...
export class ReservationsController {
  ...
  @ApiBadRequestResponse({
    description: 'Номер с указанным ID не существует или он отключён',
  })
  @ApiBadRequestResponse({
    description: 'Дата окончания не может быть меньше даты начала бронирования',
  })
  ...
  async createReservation(
    ...
    const hotelRoom = await this.hotelRoomsService.findById(
      reservationDto.hotelRoom,
    );
    ...
    if (dateEnd < dateStart) {
      throw new BadRequestException(
        'Дата окончания не может быть меньше даты начала бронирования',
      );
    }
